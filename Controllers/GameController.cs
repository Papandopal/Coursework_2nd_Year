...
public class GameController : Controller
{
    ...
    private async Task DialogWith(WebSocket webSocket)
    {
        ...
        var task_1 = model.UpdateCurrentPlayer(player.user_id);
        var task_2 = model.LoadMap();
        var task_3 = model.UpdateAll();
        ...
        try
        {
            ...
            while (!receiveResult.CloseStatus.HasValue)
            {
                ...
                else if (data.First() == "kill")
                {
                    ...
                    await model.NewSize(victim_id, killer_id);
    ...
    async void GoToMouseCoord(object obj)
    {
        ...
        var player = model.Players.Where(item => item.user_id == data.user_id).FirstOrDefault();
        ...
        if (mouse_x != player.x && mouse_y != player.y)
        {
            ...
            player.x = Math.Min(Math.Max(player.x, 0), Rules.MapWidth);
            player.mouse_x = Math.Min(Math.Max(player.mouse_x, 0), Rules.MapWidth);
            ...
            player.y = Math.Min(Math.Max(player.y, 0), Rules.MapHeight);
            player.mouse_y = Math.Min(Math.Max(player.mouse_y, 0), Rules.MapHeight);
    ...
    async Task<string> GetName(WebSocket webSocket)
    {
        var buffer = new byte[1024 * 4];
        var receiveResult = await webSocket.ReceiveAsync(
            new ArraySegment<byte>(buffer), CancellationToken.None);
        ...
        throw new Exception($"Первым пришло не имя а{data}");
    }
}
